...
namespace CursedSummit.Utils
{
    ...
    public abstract class EnumConstraint<TEnum> where TEnum : class
    {
        ...
        public static string GetNameTitleCase<T>(T value) where T : struct, TEnum
        {
            string[] words = GetName(value).Split(Underscore, StringSplitOptions.RemoveEmptyEntries);
            if (words.Length == 0) { return string.Empty; }
            for (int i = 0; i < words.Length; i++)
            {
                char[] word = words[i].ToLower().ToCharArray();
        ...
        /// <summary>
        /// Transforms an ALL_CAPS enum name to a CamelCase name
        /// </summary>
        /// <param name="name">Enum name to transform</param>
        /// <returns>The CamelCase version of the name</returns>
        public static string ToCamelCase(string name)
        {
            string[] words = name.Split(Underscore, StringSplitOptions.RemoveEmptyEntries);
            if (words.Length == 0) { return string.Empty; }
            for (int i = 0; i < words.Length; i++)
            {
                char[] word = words[i].ToLower().ToCharArray();
